...
Ciclo de vida de un \textbf{Pedido}. Se factoriza la transición \texttt{anular} mediante un
superestado \texttt{EnProceso} que agrupa los estados \texttt{Registrado}, \texttt{Confirmado},
\texttt{Enviado}, \texttt{Entregado} y \texttt{Cancelado}, de modo que el evento \texttt{anular}
puede dispararse desde cualquiera de ellos. La transición \texttt{Registrado} $\to$ \texttt{Confirmado}
lleva la condición de guarda \texttt{[stock disponible]}.

\begin{figure}[H]
    \centering
    \includegraphics[width=0.85\textwidth]{figuras/p3_diagrama_estados.png}
...
\textbf{Corrección aplicada:} se documenta que el diagrama de actividades de la P2 debe extenderse
(o complementarse con actividades adicionales ``Despachar pedido'' y ``Entregar pedido'') para
cubrir el ciclo de vida completo modelado en P3. En el alcance de esta prueba se deja registrada la
inconsistencia como hallazgo de la verificación cruzada.
